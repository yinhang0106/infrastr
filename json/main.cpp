...
using JSONDict = std::map<std::string, JSONObject>;     // todo: map & unordered_map?
...
std::pair<JSONObject, size_t> parse(std::string_view json) {
    if (json.empty()) {
        ...
        for (i = 1; i < json.size();) {
            ...
            auto key = std::move(std::get<std::string>(key_obj.inner));
...
int main() {
    std::string str = R"({"hi":100, "world":[3.45, "he"]})";
